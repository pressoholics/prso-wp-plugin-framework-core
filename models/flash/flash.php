...
class PrsoCoreFlashModel {
	...
	function __construct() {
		...
		if( is_admin() ) {
			...
			add_action( 'prso_validate', array( &$this, 'get_validate_flash' ) );
		}
	}
	...
	private function model_custom_wp_actions() {
		...
		//WP Action hook for $this->set_flash
		$this->add_action( 'prso_core_set_flash', 'set_flash', 10, 3 );
		
		//WP Action hook for $this->set_validate_flash
		$this->add_action( 'prso_core_set_validation_flash', 'set_validate_flash', 10, 1 );
	...
	private function model_custom_wp_filters() {
		...
		//WP Filter hook for $this->get_validate_flash
		$this->add_filter( 'prso_core_get_flash_script', 'get_validate_flash', 1, 1 );
	...
	* @author	Ben Moody
	*/
	public function get_validate_flash( $output_script = NULL ) {
